...
function decompressBytes(compressed: Uint8Array): Uint8Array {
    ...
    try {
        ...
        console.warn('Error descomprimiendo, retornando datos sin descomprimir:', e);
        return compressed;
    }
}
 ...
 */
function decodeArrayBinary(encoded: BinaryCompressedArray): Float32Array | Float64Array {
    ...
    if (encoded.quantized && encoded.min_val !== undefined && encoded.max_val !== undefined) {
        const shape = encoded.shape;
        const totalSize = shape.reduce((a, b) => a * b, 1);
        const quantized = dataBytes.slice(0, totalSize);
        const denormalized = dequantizeFromUint8(quantized, shape, encoded.min_val, encoded.max_val);
...
export function decompressArray(compressed: CompressedArray | BinaryCompressedArray): number[][] {
    /**
...
export function isCompressed(data: any): data is CompressedArray | BinaryCompressedArray {
    /**
...
export async function decodeBinaryFrame(data: ArrayBuffer | Uint8Array | string): Promise<any> {
    try {
        ...
        throw new Error('No se pudo decodificar el frame (no es JSON ni CBOR válido)');
    } catch (error) {
 ...
 */
export function processDecodedPayload(frameData: any, previousFrame?: any): any {
    try {
        ...
        for (const [key, encoded] of Object.entries(arrays)) {
            ...
            if (encodedArray.format === 'binary') {
                ...
                if (encodedArray.is_differential && previousFrame && previousFrame[key]) {
