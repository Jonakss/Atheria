...
export function saveFrameToTimeline(
  ...
  try {
    ...
    // Verificar si es error de cuota de manera robusta
    const isQuotaError =
    ...
    if (isQuotaError) {
      ...
      try {
        ...
        // Fallback final: borrar todo para recuperar funcionalidad
        try {
          console.warn(
            "Limpieza falló. Borrando timeline completo para recuperar espacio."
          );
