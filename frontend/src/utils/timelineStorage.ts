...
/**
 * Obtiene el tamaño aproximado de un objeto en bytes (JSON stringificado)
 */
 ...
 */
function cleanupFrames(frames: TimelineFrame[], maxFrames: number): TimelineFrame[] {
...
export function saveFrameToTimeline(
    ...
    try {
        ...
        if (dataSize > STORAGE_SIZE_LIMIT) {
            // Reducir a la mitad
            const reducedFrames = cleanupFrames(cleanedFrames, Math.floor(maxFrames / 2));
            ...
            console.warn(`Timeline excedió límite de almacenamiento. Reducido a ${reducedFrames.length} frames.`);
            return true;
        }
        ...
        if (error instanceof Error && error.name === 'QuotaExceededError') {
            ...
            try {
                const existing = loadTimeline(experimentName, Math.floor(maxFrames / 2));
                // Intentar guardar con menos frames
                if (existing.frames.length > 0) {
                    const reduced = cleanupFrames(existing.frames, Math.floor(maxFrames / 4));
                    const reducedData = {
                        metadata: {
                            max_frames: Math.floor(maxFrames / 4),
                            total_frames: reduced.length,
                            min_step: reduced.length > 0 ? Math.min(...reduced.map(f => f.step)) : 0,
                            max_step: reduced.length > 0 ? Math.max(...reduced.map(f => f.step)) : 0,
                            last_updated: Date.now(),
                        },
                        frames: reduced,
                    };
                    localStorage.setItem(getStorageKey(experimentName), JSON.stringify(reducedData));
                    return true;
                }
            } catch (e) {
 ...
 */
export function getTimelineStats(experimentName: string | null = null): TimelineMetadata | null {
    try {
        const storageKey = getStorageKey(experimentName);
        const stored = localStorage.getItem(storageKey);
...
export function getFramesRange(
    ...
    const timeline = loadTimeline(experimentName);
    return timeline.frames.filter(f => f.step >= startStep && f.step <= endStep);
}
