...
export function Complex3DViewer() {
    ...
    useEffect(() => {
        ...
        try {
            ...
            // Primera pasada: encontrar rangos
            ...
            // Segunda pasada: crear puntos
            frames.forEach((frame, timeIndex) => {
                if (!frame.real || !frame.imag) return;
                const H = frame.real.length;
                const W = frame.real[0]?.length || 0;
                ...
                for (let i = 0; i < H; i += step) {
                    for (let j = 0; j < W; j += step) {
                        ...
                        const z = (timeIndex / frames.length - 0.5) * scale * 2;
                        
                        positions.push(x, y, z);
                        
                        // Color basado en tiempo (gradiente azul a rojo)
                        const t = timeIndex / frames.length;
    ...
    }, [historyFrames, loadedHistoryFrames, useLoadedHistory, pointSize]);
        ...
        <Paper p="md" withBorder style={{ backgroundColor: 'var(--mantine-color-dark-7)', height: '100%', display: 'flex', flexDirection: 'column' }}>
            <Stack gap="sm" style={{ flex: 1, minHeight: 0 }}>
                ...
                <Text size="xs" c="dimmed" style={{ fontStyle: 'italic' }}>
                    Eje X (rojo): Parte Real, Eje Y (verde): Parte Imaginaria, Eje Z (azul): Tiempo. 
                    {frames.length > 0 && ` Mostrando ${frames.length} frames.`}
                </Text>
            </Stack>
        </Paper>
    );
