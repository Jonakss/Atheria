...
async def handle_play(args):
    ...
    if motor_is_native and hasattr(motor, "get_dense_state"):
        ...
        if grid_size > 128:
            logging.info(
                f"⏩ Saltando frame inicial para motor nativo (grid={grid_size} > 128) para evitar bloqueo."
            )
        else:
            try:
                ...
                logging.info(
                    "📤 Intentando enviar frame inicial (mejor esfuerzo, no bloqueante)..."
                )
                loop = asyncio.get_event_loop()
                psi = await asyncio.wait_for(
                    loop.run_in_executor(
                        None,
                        lambda: motor.get_dense_state(
                            check_pause_callback=lambda: g_state.get("is_paused", False)
                        ),
                    ),
                ...
                if (
                    psi is not None
                    and isinstance(psi, torch.Tensor)
                    and psi.numel() > 0
                ):
                    ...
                    if psi_abs_max > 1e-10:
                        ...
                        delta_psi = (
                            motor.last_delta_psi
                            if hasattr(motor, "last_delta_psi")
                            else None
                        )
                        viz_data = get_visualization_data(
                            psi, viz_type, delta_psi=delta_psi, motor=motor
                        )
                            ...
                            if has_data:
                                ...
                                if range_val > 1e-6:
                                    ...
                                    frame_payload_raw = {
                                        ...
                                        "poincare_coords": viz_data.get(
                                            "poincare_coords", []
                                        ),
                                        "phase_attractor": viz_data.get(
                                            "phase_attractor"
                                        ),
                                        "flow_data": viz_data.get("flow_data"),
                                        "phase_hsv_data": viz_data.get(
                                            "phase_hsv_data"
                                        ),
                                        "complex_3d_data": viz_data.get(
                                            "complex_3d_data"
                                        ),
                                        "simulation_info": {
                                            "step": current_step,
                                            "is_paused": False,
                                            "live_feed_enabled": g_state.get(
                                    ...
                                    frame_payload = await optimize_frame_payload(
                                        frame_payload_raw,
                                        enable_compression=g_state.get(
                                            "data_compression_enabled", True
                                        ),
                                        downsample_factor=g_state.get(
                                            "downsample_factor", 1
                                        ),
                                        viz_type=viz_type,
                                    )
                                    ...
                                    logging.info(
                                        f"📤 Frame inicial enviado al frontend (step={current_step})"
                                    )
            except asyncio.TimeoutError:
                # Timeout es aceptable - la visualización se actualizará en el siguiente step
                logging.warning(
                    "⏱️ Timeout enviando frame inicial (30s). La visualización se actualizará después."
                )
            except Exception as e:
                # Error es aceptable - la visualización se actualizará en el siguiente step
                logging.warning(
                    f"⚠️ Error enviando frame inicial: {e}. La visualización se actualizará después."
                )
