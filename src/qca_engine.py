...
class Aetheria_Motor:
    ...
    def evolve_step(self):
        """Evoluciona el estado usando la Ley M unitaria."""
        with torch.no_grad():
            # 1. Preparar el estado (B, C, H, W)
            x_cat_total = self.state.get_cat_input()
            
            # 2. La Ley M (U-Net) predice el Delta (la derivada)
            # La U-Net está diseñada para que este delta sea una rotación
            delta_psi = self.operator(x_cat_total) # Salida: [B, C, H, W]
            
            # 3. Aplicar el Método de Euler
            # (Delta ya está en formato (B, C, H, W), lo permutamos)
