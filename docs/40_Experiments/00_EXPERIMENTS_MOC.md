# 📊 Experiments - Map of Content

> Mapa de contenido para experimentos y resultados de Atheria 4

---

## 📝 Guías y Tutoriales

- **[Cómo Ejecutar](../40_Experiments/HOW_TO_RUN.md)** - Instrucciones completas de ejecución
- **[Guía de Experimentación](../40_Experiments/EXPERIMENTATION_GUIDE.md)** - Cómo experimentar efectivamente
- **[Guía de Pruebas de Visualización](../40_Experiments/VISUALIZATION_TESTING.md)** - Tests de visualizaciones
- **[Entrenamiento Progresivo](../40_Experiments/PROGRESSIVE_TRAINING.md)** - Técnicas de entrenamiento

---

## 🎨 UI/UX Improvements

- **[UI Dashboard Improvements](../40_Experiments/UI_DASHBOARD_IMPROVEMENTS_2025_11_29.md)** - Scientific Metrics & UI Investigation (2025-11-29)
- **[UI Review Full](../40_Experiments/UI_IMPROVEMENTS_2025_11_29.md)** - Complete UI analysis with 10 suggestions
- **[Training Guide: EMERGE_TEST](../40_Experiments/TRAINING_EMERGE_TEST_2240.md)** - Experiment guide for emergence testing
- **[Low-Memory Training Guide](../40_Experiments/LOW_MEMORY_TRAINING_GUIDE.md)** - Optimization for GPUs with <4 GiB VRAM

---

## 🔍 External Research

- **[Gemini Optimization Analysis](../40_Experiments/EXTERNAL_RESEARCH_GEMINI_OPTIMIZATION_ANALYSIS.md)** - External AI analysis on model serving optimization strategies (LitServe, torch.compile, quantization)

---

## 📓 Bitácoras

- **[Log de Desarrollo AI](../40_Experiments/AI_DEV_LOG.md)** - Bitácora de desarrollo con IA
- **[Log de Setup Fase 2](../40_Experiments/PHASE_2_SETUP_LOG.md)** - Setup del Motor Nativo C++
- **[Test de Motor Nativo con Modelos Reales](../40_Experiments/PHASE_2_NATIVE_ENGINE_TEST.md)** - Script de prueba con modelos entrenados
- **[Informe de Estado de Fases](../40_Experiments/PHASE_STATUS_REPORT.md)** - Estado actual de todas las fases y componentes

---

## 🔬 Experimentos C++ y Motor Nativo

- **[EXP_002: Motor C++](../40_Experiments/EXP_002_CPP_ENGINE.md)** - Primer motor C++
- **[EXP_003: Integración LibTorch](../40_Experiments/EXP_003_CPP_LIBTORCH.md)** - Integración con LibTorch
- **[EXP_004: Benchmark V1](../40_Experiments/EXP_004_BENCHMARK_CPP_V1.md)** - Benchmark del motor C++ V1
- **[EXP_005: Victoria Motor Nativo](../40_Experiments/EXP_005_CPP_NATIVE_VICTORY.md)** - Resultados del motor nativo

---

## 📈 Benchmarks y Rendimiento

- **[Benchmark de Almacenamiento de Tensores](../40_Experiments/BENCHMARK_TENSOR_STORAGE.md)** - Comparación de almacenamiento
- **[Benchmark Python vs Native](../40_Experiments/BENCHMARK_PYTHON_VS_NATIVE.md)** - Comparación de rendimiento Python vs C++ Nativo
- **[EXP_006: Optimización de Transferencia](../40_Experiments/EXP_006_DATA_TRANSFER_OPTIMIZATION.md)** - Optimización de transferencia de datos
- **[EXP_007: Verificación de Spatial Indexing](../40_Experiments/EXP_007_SPATIAL_INDEXING_VERIFICATION.md)** - Verificación de códigos Morton
- **[Análisis de Optimización de Visualización](../40_Experiments/VISUALIZATION_OPTIMIZATION_ANALYSIS.md)** - Análisis de flujo de visualización y opciones de optimización
- **[Evaluación Arquitectónica: Go vs Python](../40_Experiments/ARCHITECTURE_EVALUATION_GO_VS_PYTHON.md)** - Análisis de usar Go para comunicaciones

---

## 🔗 Compatibilidad e Integración

- **[Compatibilidad V3/V4/Nativo](../40_Experiments/COMPATIBILITY_V3_V4_NATIVE.md)** - Compatibilidad entre versiones
- **[Integración Frontend Motor Nativo](../40_Experiments/NATIVE_ENGINE_FRONTEND_INTEGRATION.md)** - Integración frontend

---

## 🔗 Enlaces Relacionados

- [[10_core/00_CORE_MOC]] - Documentación core
- [[30_Components/00_COMPONENTS_MOC]] - Componentes técnicos

---

## 📌 Tags

#experiment #moc #benchmark #results #guide

